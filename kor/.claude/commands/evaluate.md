...
2. 각 버전(v1, v2, v3)에 대해:
   ...
   b. 빌드 테스트: cd versions/vN && npm run build
      - npm run build 실패 시: 해당 버전은 "빌드 실패"로 기록, 기능 점수 0점 처리
      - 평가는 나머지 버전으로 계속 진행
      - 모든 버전이 빌드 실패면 사용자에게 안내 후 중단
   ...
   g. **Playwright 브라우저 테스트** (MCP 사용 가능 시):
      - 각 버전을 순차적으로 테스트 (동시 실행 X):
        v1: PORT=3001 npm run dev → 테스트 → 종료
        v2: PORT=3002 npm run dev → 테스트 → 종료
        v3: PORT=3003 npm run dev → 테스트 → 종료
      - dev 서버 시작 후 3초 대기, 실패 시 5초 재시도 (최대 2회)
      - Playwright MCP로 메인 페이지 스크린샷 캡처
      - 주요 링크/버튼 클릭 테스트 수행
      - 테스트 결과와 스크린샷을 evaluation/에 저장
      - dev 서버 종료 확인 후 다음 버전 진행
