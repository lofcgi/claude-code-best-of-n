...
| MCP 서버 | 설치 방법 | 스킬 연동 |
|----------|----------|----------|
| **Design Inspiration** | `tools/design-inspiration-mcp` clone + build + `SERPER_API_KEY` | `/prototype` 디자인 레퍼런스 |
...
1. Design Inspiration MCP 서버 빌드:
   ```bash
   cd tools/design-inspiration-mcp && npm install && npm run build
   ```
2. [serper.dev](https://serper.dev)에서 Serper API 키 발급
3. 환경 변수로 설정: `export SERPER_API_KEY="your-key-here"`
