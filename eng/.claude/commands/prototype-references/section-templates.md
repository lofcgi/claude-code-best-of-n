# Section Templates (Code Templates per Section + Visual Density Requirements)

All sections must include SectionWrapper or at least a 2-layer background.
Sections with only text + icons are strictly prohibited.

---

## SectionWrapper (Common to All Sections)

A wrapper component that enforces at least a 2-layer background on every section.
Place it in `components/sections/section-wrapper.tsx`.
...
- 2x2 large cards (full-bleed images) + 1x1 small cards
- SpotlightCard hover effect required
- Each card must have an image or CSS visual (text + icon only is prohibited)
...
**Visual Density Requirements:**
- Glass-morphism container (backdrop-blur-md, bg-white/5, border-white/10)
- Gradient header (primary to accent)
- Check/X icons + primary glow on check
...
**Visual Density Requirements:**
- 64px avatar images (Unsplash)
- Quote + name/role
- InfiniteMarquee or 3-column grid
...
**Visual Density Requirements:**
- Number + icon + **product screenshot** (alternating left/right layout)
- Primary glow on numbers
- Screenshots inside BrowserMockup or PhoneMockup
- Connecting lines (SVG path or dashed border)
...
- Full-bleed gradient background (primary to surface)
- Decorative elements (Sparkles, floating shapes)
- Specific numbers (10,847 not 10,000)
...
**Visual Density Requirements:**
- InfiniteMarquee or static logo grid
...
- Open/close animation (spring physics)
...
**Visual Density Requirements:**
- 3 tier cards (middle card emphasized — scale, glow border)
- NumberTicker on prices
- Glow box-shadow on CTA
- "Most Popular" badge
...
- Dual CTA buttons (Primary glow + Secondary ghost)
- Background decorations (Sparkles or BackgroundBeams)
...
1. **All sections must have SectionWrapper or at least a 2-layer background**
2. **Feature cards without images are prohibited**
3. **All cards must have hover effects (scale + border glow)**
4. **Spring physics only (linear is prohibited)**
5. **Use specific numbers (10,847 not 10,000)**
6. **py-40 minimum (py-24/py-32 prohibited)**
7. **No function definitions inside page files (only export default allowed)**
8. **All components must be imported from components/premium/ or components/sections/**
