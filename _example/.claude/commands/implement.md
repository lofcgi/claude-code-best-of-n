...
### Step 0.8 — 배포 플랫폼 제약사항 조회

tech_stack.deploy 값을 기반으로, **해당 플랫폼의 공식 문서**를 Context7 또는 웹에서 조회한다.
학습 지식에 의존하지 않고, 반드시 공식 문서에서 최신 수치를 확인하여
`analysis/tech-constraints.md`에 **"배포 플랫폼 제약"** 섹션으로 추가:

조회 필수 항목 (플랫폼 무관, 모든 배포 대상에 적용):
1. **서버리스 함수 요청 Body 크기 제한** — 최대 수신 가능 크기 (숫자 + 출처 URL)
2. **API 응답 크기 제한** — 서버리스 함수 응답 최대 크기
3. **함수 실행 시간 제한** — 플랜별 타임아웃 (무료/유료)
4. **파일 저장소 SDK 권장 패턴** — 해당 플랫폼의 파일 스토리지 서비스(Blob, R2, S3 등)가 있으면:
   - 클라이언트 직접 업로드 패턴이 존재하는지 (body 제한 우회용)
   - 업로드 시 충돌 방지 옵션 (random suffix, dedup 등)
   - 업로드 직후 읽기 시 eventual consistency 여부 및 권장 대응 패턴
5. **기타 플랫폼 고유 제약** — 콜드 스타트, 리전 제한, 동시 실행 수 등

**원칙**: 수치는 반드시 공식 문서 출처와 함께 기록한다.
이후 Phase 3의 모든 기능 구현 시 이 섹션을 참조해야 한다.
...
2.5. 플랫폼 제약 체크 (모든 기능):
   a. 이 기능이 파일 업로드/다운로드를 포함하면 → Step 0.8 제약 + 복잡 기능 표 "파일 업로드" 행 참조
   b. request/response 크기가 Step 0.8에서 확인한 플랫폼 제한을 초과할 수 있으면 → 해당 플랫폼 공식문서의 우회 패턴(클라이언트 업로드, 스트리밍 등) 적용
   c. 서버 응답 파싱 시 JSON 실패 대비 try-catch 필수 — 모든 서버리스 플랫폼은 자체 에러 페이지(HTML/plain-text)를 반환할 수 있음
   d. 확신이 없으면 Context7으로 해당 플랫폼+기능 키워드 조회하여 공식 패턴 확인
...
5.5. 런타임 스모크 테스트 (파일 업로드 또는 외부 API 연동 기능에만):
   - Playwright 가능 시: dev 서버 시작 → 해당 기능 1회 실행 → 성공 확인 → dev 서버 종료
   - 업로드 기능: 작은 파일(~1MB) + 큰 파일(body 제한의 80%)로 2회 테스트
   - API 연동: 실제 API 1회 호출하여 응답 구조 확인
   - 실패 시: 에러 메시지 JSON 여부 확인, non-JSON이면 클라이언트 파싱 보강
...
| 기능 유형 | 특수 고려사항 |
|---|---|
| 파일 업로드 | **Step 0.8의 플랫폼 제약 필수 참조.** ① body 제한 < 업로드 max size이면 해당 플랫폼의 **클라이언트 직접 업로드 패턴**을 Context7/공식문서에서 조회하여 적용 (서버 경유 금지). ② 스토리지 SDK의 충돌 방지 옵션(random suffix 등)을 공식문서에서 확인 후 필수 적용. ③ 업로드 직후 읽기 시 retry 로직 포함 (eventual consistency 대응). ④ 클라이언트 에러 파싱은 non-JSON 응답 대비 try-catch 필수 (플랫폼이 HTML/plain-text 에러를 반환할 수 있음). |
| 클라우드 스토리지 읽기 | 업로드 직후 fetch 시 404/stale 가능 (eventual consistency는 대부분의 CDN/오브젝트 스토리지 공통). 최소 3회 retry + 지수 백오프 적용. 해당 SDK 공식문서에서 일관성 관련 옵션 확인 후 적용. |
...
### Step 4.6 — 핵심 기능 E2E 검증 (인터랙티브 포함)

requirements.json의 must-have 기능 각각에 대해:

1. Playwright로 전체 사용자 플로우 실행 (입력 → 처리 → 결과)
...
3. **인터랙티브 요소 개별 테스트**:
   - **미디어 플레이어**: 재생 → 일시정지 → seek bar 클릭 → 재생 재개.
     비디오+오디오 별도 요소이면 **양쪽 동기화** 확인
   - **토글/접기**: 펼침 → 내용 확인 → 접기
   - **다운로드**: 클릭 → 네트워크 요청 또는 href 존재 확인
   - **폼 입력**: 드롭다운 변경 → 연관 UI 업데이트 확인
   - **긴 텍스트**: overflow 처리 확인 (line-clamp, 스크롤, 접기)

4. 문제 발견 시 즉시 수정 → 재실행 (최대 3회)

검증 불가 기능 (OAuth 등): 수동 테스트 체크리스트 생성
